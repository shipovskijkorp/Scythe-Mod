...
public final class HudSync {
    private static HudTransport<ServerPlayerEntity> transport;
    private HudSync() {}

    public static void install(HudTransport<ServerPlayerEntity> implementation) {
    ...
    public static void start(ServerPlayerEntity player, HudTransport.Timer timer, int ticksLeft) {
        transport().start(player, timer, Math.max(0, ticksLeft));
    }

    public static void stop(ServerPlayerEntity player, HudTransport.Timer timer) {
