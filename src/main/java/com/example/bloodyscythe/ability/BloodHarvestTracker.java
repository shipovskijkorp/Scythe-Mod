...
public class BloodHarvestTracker {
    ...
    public static int start(ServerPlayerEntity player) {
        ...
        if (BloodyScytheConfigLoader.CONFIG != null) {
            ticks = Math.max(1, BloodyScytheConfigLoader.CONFIG.bloodHarvestKillWindowTicks);
        }
    ...
    public static void tick(ServerPlayerEntity player) {
        ...
        if (time <= 0) {
            ...
            if (BloodyScytheConfigLoader.CONFIG != null) {
                debuffTicks = Math.max(1, BloodyScytheConfigLoader.CONFIG.bloodHarvestFailureDebuffTicks);
                slowAmp = Math.max(0, BloodyScytheConfigLoader.CONFIG.bloodHarvestFailureSlownessAmplifier);
                weakAmp = Math.max(0, BloodyScytheConfigLoader.CONFIG.bloodHarvestFailureWeaknessAmplifier);
            }
            ...
            player.sendMessage(Text.translatable("message.bloodyscythe.blood_harvest.failed"), true);
