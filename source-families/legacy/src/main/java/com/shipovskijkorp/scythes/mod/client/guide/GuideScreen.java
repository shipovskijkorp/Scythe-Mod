...
/** Compact BCCE-style Guide Book screen for the legacy family. */
public final class GuideScreen extends Screen {
    ...
    private static Identifier id(String path) {
        return new Identifier(ScytheMod.MOD_ID, path);
    }
    ...
    private void renderTab(DrawContext context, int mouseX, int mouseY, int row, String rawLabel,
        ...
        int textY = y + 4;
        context.drawText(textRenderer, label, textX, textY, GuideUi.TEXT, false);
    ...
    private int renderWrapped(DrawContext context, String text, int x, int y, int bottom) {
        ...
        for (OrderedText line : lines) {
            if (y + 9 > bottom) break;
            context.drawText(textRenderer, line, x, y, GuideUi.TEXT, false);
            y += 10;
        }
        return y + 4;
    }
    ...
    private void renderRecipeTooltip(DrawContext context, int mouseX, int mouseY) {
        ...
        context.drawText(textRenderer, hoveredRecipeName, x + 4, y + 3, GuideUi.opaque(0xFFFFFF), false);
    }
    private int renderScytheIndex(DrawContext context, int x, int y, int mouseX, int mouseY, int bottom) {
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            String label = view.resources().document(entry.page()).title();
            context.drawText(textRenderer, label, x + 25, y + 6, GuideUi.TEXT, false);
            final int entryIndex = i;
    ...
    private int renderChapter(DrawContext context, String title, int x, int y) {
        ...
        int textX = x + 16;
        context.drawText(textRenderer, title, textX, y, GuideUi.TEXT, false);
