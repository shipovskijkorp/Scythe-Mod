...
public final class FireScytheLockHighlighter {
    ...
    public static void tick(MinecraftClient client) {
        ...
        if (!wasDrawing) {
            lockedTarget = FireTargeting.findLockTarget(client.player);
        }
        wasDrawing = true;

        if (lockedTarget != null && (!lockedTarget.isAlive() || lockedTarget.getWorld() != client.world)) {
            lockedTarget = null;
        }
    }

    public static boolean isLocked(Entity entity) {
        return entity == lockedTarget;
    }
