...
public class ToxicScytheItem extends ScytheSwordItem {
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if (!attacker.getWorld().isClient) {
            ...
            if (attacker.getRandom().nextDouble() < ScytheBalance.Toxic.PASSIVE_ARMOR_DAMAGE_CHANCE) {
                int armorDamage = applyAcidityArmorDamageBonus(ScytheBalance.Toxic.PASSIVE_ARMOR_DAMAGE, getAcidityLevel(stack), attacker.getRandom());
    ...
    public TypedActionResult<ItemStack> use(World world, net.minecraft.entity.player.PlayerEntity user, Hand hand) {
        ...
        stack.damage(ScytheBalance.Toxic.ORB_DURABILITY_COST, player, p -> p.sendToolBreakStatus(hand));
