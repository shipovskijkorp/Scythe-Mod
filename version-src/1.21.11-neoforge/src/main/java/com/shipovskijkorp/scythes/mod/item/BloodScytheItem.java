...
public class BloodScytheItem extends ScytheSwordItem {
    ...
    private static void pullTowardPlayer(ServerPlayerEntity player, LivingEntity target) {
        Vec3d toPlayer = new Vec3d(player.getX() - target.getX(), player.getBodyY(0.5D) - target.getBodyY(0.5D), player.getZ() - target.getZ());
    ...
    private static int getSpikedBladeLevel(LivingEntity holder, ItemStack stack) {
        Optional<RegistryEntry.Reference<Enchantment>> entry = holder.getEntityWorld()
                .getRegistryManager()
                .getOrThrow(RegistryKeys.ENCHANTMENT)
                .getOptional(SPIKED_BLADE_KEY);
