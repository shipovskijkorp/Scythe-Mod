...
public class ToxicScytheItem extends Item {
    ...
    public void postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if (attacker.getRandom().nextDouble() < PASSIVE_ARMOR_DAMAGE_CHANCE) {
            int armorDamage = applyAcidityArmorDamageBonus(PASSIVE_ARMOR_DAMAGE, getAcidityLevel(attacker, stack), attacker.getRandom());
    ...
    public ActionResult use(World world, net.minecraft.entity.player.PlayerEntity user, Hand hand) {
        ...
        stack.damage(ORB_DURABILITY_COST, player, hand == Hand.MAIN_HAND ? EquipmentSlot.MAINHAND : EquipmentSlot.OFFHAND);
    ...
    public static int getAcidityLevel(LivingEntity holder, ItemStack stack) {
        return Math.max(0, Math.min(3, holder.getEntityWorld()
                .getRegistryManager()
