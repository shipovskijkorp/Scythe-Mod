...
public final class ScytheTooltips {
    ...
    private static void blood(ItemStack stack, Item.TooltipContext context, TooltipDisplayComponent displayComponent, Consumer<Text> textConsumer, TooltipType type) {
        ...
        if (!alt) {
            ...
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.bleed_chance_percent", TooltipUtil.fmtPercentValue(ScytheBalance.Blood.BLEEDING_CHANCE)), Formatting.DARK_RED);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.bleed_base_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.Blood.BLEEDING_BASE_DURATION_TICKS)), Formatting.DARK_GRAY);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.bleed_extend_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.Blood.BLEEDING_EXTEND_TICKS)), Formatting.DARK_GRAY);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.bleeding_damage_per_second", TooltipUtil.fmtNumber(ScytheBalance.Bleeding.DAMAGE_PER_SECOND)), Formatting.DARK_GRAY);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.vampirism_chance_percent", TooltipUtil.fmtPercentValue(ScytheBalance.Vampirism.VAMPIRISM_CHANCE)), Formatting.DARK_RED);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.vampirism_heal_percent", TooltipUtil.fmtPercentValue(ScytheBalance.Vampirism.HEAL_FRACTION)), Formatting.DARK_GRAY);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.vampirism_cooldown_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.Vampirism.COOLDOWN_TICKS)), Formatting.DARK_GRAY);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.defense_pierce", TooltipUtil.fmtPercentValue(ScytheBalance.Blood.DEFENSE_PIERCE_CHANCE), TooltipUtil.fmtPercentValue(ScytheBalance.Blood.DEFENSE_PIERCE_MITIGATION_IGNORED)), Formatting.DARK_RED);
        }
        ...
        if (!alt) {
            ...
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.radius_blocks", TooltipUtil.fmtNumber(ScytheBalance.Blood.BLENDER_RADIUS)), Formatting.GRAY);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.cooldown_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.Blood.BLENDER_COOLDOWN_TICKS)), Formatting.GRAY);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.durability_cost", String.valueOf(ScytheBalance.Blood.BLENDER_DURABILITY_COST)), Formatting.GRAY);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.basic_hit_count", String.valueOf(ScytheBalance.Blood.BLENDER_HIT_COUNT)), Formatting.DARK_GRAY);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.slowness_ii_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.Blood.BLENDER_SLOWNESS_TICKS)), Formatting.DARK_GRAY);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.bleeding_ii_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.Blood.BLENDER_BLEEDING_TICKS)), Formatting.DARK_GRAY);
        }
        ...
        TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.radius_blocks", TooltipUtil.fmtNumber(ScytheBalance.BloodHarvest.RADIUS)), Formatting.GRAY);
        TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.cooldown_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.BloodHarvest.COOLDOWN_TICKS)), Formatting.GRAY);
        TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.durability_cost", String.valueOf(ScytheBalance.BloodHarvest.DURABILITY_COST)), Formatting.GRAY);
        TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.kill_window_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.BloodHarvest.KILL_WINDOW_TICKS)), Formatting.GRAY);
        TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.slowness_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.BloodHarvest.SLOWNESS_TICKS)), Formatting.DARK_GRAY);
        TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.blindness_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.BloodHarvest.BLINDNESS_TICKS)), Formatting.DARK_GRAY);
        TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.weakness_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.BloodHarvest.WEAKNESS_TICKS)), Formatting.DARK_GRAY);
        TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.glowing_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.BloodHarvest.GLOWING_TICKS)), Formatting.DARK_GRAY);
        TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.success_buffs_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.BloodHarvest.SUCCESS_BUFF_TICKS)), Formatting.DARK_GRAY);
        TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.failure_debuffs_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.BloodHarvest.FAILURE_DEBUFF_TICKS)), Formatting.DARK_GRAY);

        TooltipUtil.flush(tooltip, textConsumer);
    }

    private static void toxic(ItemStack stack, Item.TooltipContext context, TooltipDisplayComponent displayComponent, Consumer<Text> textConsumer, TooltipType type) {
        List<Text> tooltip = new ArrayList<>();
    ...
    private static void withering(ItemStack stack, Item.TooltipContext context, TooltipDisplayComponent displayComponent, Consumer<Text> textConsumer, TooltipType type) {
        List<Text> tooltip = new ArrayList<>();
        ...
        if (!alt) {
            ...
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.minion_cap", String.valueOf(WitheringScytheItem.getMaxMinions(context, stack))), Formatting.GRAY);
    ...
    private static void golden(ItemStack stack, Item.TooltipContext context, TooltipDisplayComponent displayComponent, Consumer<Text> textConsumer, TooltipType type) {
        List<Text> tooltip = new ArrayList<>();
    ...
    private static void fire(ItemStack stack, Item.TooltipContext context, TooltipDisplayComponent displayComponent, Consumer<Text> textConsumer, TooltipType type) {
        List<Text> tooltip = new ArrayList<>();
        TooltipUtil.addWrapped(tooltip, "tooltip.scythes.fire_scythe.desc", Formatting.GRAY, Formatting.ITALIC);
        if (!TooltipUtil.isShiftDown()) { TooltipUtil.addHoldShiftHint(tooltip); TooltipUtil.flush(tooltip, textConsumer); return; }
        boolean alt = TooltipUtil.isAltDown();
        if (alt) { tooltip.add(Text.empty()); TooltipUtil.addWrapped(tooltip, "tooltip.scythes.fire_scythe.base_stats", Formatting.DARK_GRAY); }

        tooltip.add(Text.empty());
        tooltip.add(Text.translatable("tooltip.scythes.section.passive").formatted(Formatting.GRAY));
        tooltip.add(Text.translatable("tooltip.scythes.fire_scythe.passive").formatted(Formatting.GOLD));
        if (!alt) TooltipUtil.addWrapped(tooltip, "tooltip.scythes.fire_scythe.passive.desc", Formatting.DARK_GRAY);
        else {
            TooltipUtil.addWrapped(tooltip, "tooltip.scythes.stat.lava_walk", Formatting.DARK_GRAY);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.burns_chance_percent", TooltipUtil.fmtPercentValue(ScytheBalance.Fire.PASSIVE_BURNS_CHANCE)), Formatting.GOLD);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.burns_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.Fire.PASSIVE_BURNS_TICKS)), Formatting.DARK_GRAY);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.burns_immunity", TooltipUtil.fmtSecondsValue(ScytheBalance.Burns.MAX_CONTINUOUS_TICKS), TooltipUtil.fmtSecondsValue(ScytheBalance.Burns.IMMUNITY_TICKS)), Formatting.DARK_GRAY);
        }

        tooltip.add(Text.empty());
        tooltip.add(Text.translatable("tooltip.scythes.section.special").formatted(Formatting.GRAY));
        tooltip.add(Text.translatable("tooltip.scythes.fireball").formatted(Formatting.GOLD));
        if (!alt) TooltipUtil.addWrapped(tooltip, "tooltip.scythes.fireball.desc", Formatting.GRAY);
        else {
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.cooldown_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.Fire.FIREBALL_COOLDOWN_TICKS)), Formatting.GRAY);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.durability_cost", String.valueOf(ScytheBalance.Fire.FIREBALL_DURABILITY_COST)), Formatting.GRAY);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.max_explosion_damage", TooltipUtil.fmtNumber(ScytheBalance.Fire.FIREBALL_MAX_DAMAGE)), Formatting.DARK_GRAY);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.fireball_lifetime_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.Fire.FIREBALL_MAX_LIFETIME_TICKS)), Formatting.DARK_GRAY);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.burns_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.Fire.FIREBALL_BURNS_TICKS)), Formatting.DARK_GRAY);
            TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.stun_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.Fire.FIREBALL_STUN_TICKS)), Formatting.DARK_GRAY);
        }

        tooltip.add(Text.empty());
        tooltip.add(Text.translatable("tooltip.scythes.section.active").formatted(Formatting.GRAY));
        tooltip.add(Text.translatable("tooltip.scythes.fire_burst", TooltipUtil.getScytheAbilityKeyText(Formatting.GOLD)));
        if (!alt) { TooltipUtil.addWrapped(tooltip, "tooltip.scythes.fire_burst.desc", Formatting.GRAY); TooltipUtil.addHoldAltHint(tooltip); TooltipUtil.flush(tooltip, textConsumer); return; }
        TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.radius_blocks", TooltipUtil.fmtNumber(ScytheBalance.FireBurst.RADIUS)), Formatting.GRAY);
        TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.cooldown_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.FireBurst.COOLDOWN_TICKS)), Formatting.GRAY);
        TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.durability_cost", String.valueOf(ScytheBalance.FireBurst.DURABILITY_COST)), Formatting.GRAY);
        TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.burns_sec", TooltipUtil.fmtSecondsValue(ScytheBalance.FireBurst.BURNS_TICKS)), Formatting.DARK_GRAY);
        TooltipUtil.addWrapped(tooltip, Text.translatable("tooltip.scythes.stat.armor_damage", String.valueOf(ScytheBalance.FireBurst.ARMOR_DAMAGE)), Formatting.DARK_GRAY);
        TooltipUtil.addWrapped(tooltip, "tooltip.scythes.stat.ignores_pets_and_teammates", Formatting.DARK_GRAY);
        TooltipUtil.flush(tooltip, textConsumer);
    }
